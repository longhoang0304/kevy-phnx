...
impl CommandExecutor for DecrByFloat {
    fn execute(storage: &mut Box<dyn Storage>, cmd: &Command) -> Result<CommandResult, Box<dyn Error>> {
        ...
        let new_entry = match data {
            StorageData::String(v) => Some(DecrByFloat::process_string(&entry, v, amount)?),
            StorageData::Number(v) => Some(DecrByFloat::process_number(&entry, v, amount)?),
            StorageData::Decimal(v) => Some(DecrByFloat::process_decimal(&entry, v, amount)?),
            _ => None,
        };
